...
In part one we decided on an a set of minimum cluster sizes for our final analysis based on three possible selection criteria. We then used these values in the [Jupyter notebook](https://github.com/HullUni-bioinformatics/Kitson_et_al_NMB/blob/master/notebook/OPM_NMB_conservative.ipynb) to run a final analysis for each minimum cluster size. Now we need to examine this output, check that negatives are clean and examine the contents of each well.
...
### Plot above as a svg
svg(file="diagrams/Figure7_run2.svg", 12, 8)
sample.types
dev.off()
