...
read.stats<-read.csv(file="data/combined_read_stats_run2.csv", stringsAsFactors=FALSE, header=TRUE)
...
svg(file="diagrams/coverage_run2.svg",12,8)
...
### combine the plots and write them to an svg file
svg(file="diagrams/negatives_coverage_run2.svg",12,8)
...
###Plotting cumulative read depth across samples
The per well read depth for a minimum cluster coverage of one is unexpected. We would expect that more reads would be retained at a minimum cluster size of one read than a minimum cluster size of six but clearly fewer reads are being retained. To examine this further we will plot the cumulative total reads both for all minimum cluster sizes and for just one and six reads.

```{r}
### Create a cumulative sum variable for each level of minimum cluster coverage
#read.stats$csum <- ave(read.stats$queries, read.stats$clusters_min_cov, FUN=cumsum)

### Creat a plotting order variable
#read.stats$order<-rep(seq(1,nrow(subset(read.stats, #clusters_min_cov=="1")),1),times=length(unique(read.stats$clusters_min_cov)))

### Set up the ggplot
#cumulative.all<-ggplot(read.stats, aes(x=order, y=csum, group=as.factor(clusters_min_cov))) +
  ### Add and colour the cumulative sum lines
#  geom_line(aes(colour=as.factor(clusters_min_cov))) +
  ### Give it some nice colours
#  scale_color_discrete(name="Minimum cluster coverage", l=45, h=c(0,200)) +
  ### add a sensible y axis label
#  labs(y = "Cumulative read depth", x="Number of samples included") +
  ### rotate the x-axis labels and resize the text for the svg
#  theme(axis.text.x = element_text(size = rel(1), colour = "black", angle=90))

#cumulative.subs<-ggplot(subset(read.stats, clusters_min_cov==c("1","6","11")), aes(x=order, y=csum, group=as.factor(clusters_min_cov))) +
  ### Add and colour the cumulative sum lines
#  geom_line(aes(colour=as.factor(clusters_min_cov))) +
  ### Give it some nice colours
#  scale_color_discrete(name="Minimum cluster coverage", l=45, h=c(0,200)) +
...
#svg(file="diagrams/cumulative_read_depth_run2.svg",12,8)
#grid.arrange(cumulative.all, cumulative.subs, ncol=1)
#dev.off()
...
###Conclusions
Looking at the plot for per well read depth for all samples suggests that a minimum cluster size between 46 and 61 reads results in the removal of rare sequences. It is harder to see a pattern in clusters retained as the y axis is heavily skewed but the number of clusters retained with a minimum read depth of one. Similarly we can see from the clusters and retained reads in the negatives, it is clear that removing any clusters smaller than 41 reads effectively excludes low level background contamination. These values suggest that a minimum cluster size of about 50 reads will be sufficient to remove background noise in our dataset.
...
final.read.stats<-read.csv(file="data/metaBEAT_read_stats_run2.csv", stringsAsFactors = FALSE, header = TRUE)
...
### Plot above as a svg
svg(file="diagrams/trimming_summary_moths_only_run2.svg",12,8)
samples.only
dev.off()
...
### Plot above as a svg
svg(file="diagrams/trimming_summary_sample_types_run2.svg", 12, 8)
sample.types
dev.off()
...
paste("Maximum raw read count =", max(counting$total))
paste("Minimum raw read count =", min(counting$total))
paste("Mean raw read count =", round(mean(counting$total),1))
paste("Standard deviation of raw read count =", round(sd(counting$total),1))
paste("Total raw reads =", sum(counting$total))

paste("Maximum trimmed read count =", max(counting$trimmed.total))
paste("Minimum trimmed read count =", min(counting$trimmed.total))
paste("Mean trimmed read count =", round(mean(counting$trimmed.total),1))
paste("Standard deviation of trimmed read count =", round(sd(counting$trimmed.total),1))
paste("Total trimmed reads =", sum(counting$trimmed.total))

paste("Maximum retained cluster count =", max(counting$cluster_above_thres))
paste("Minimum retained cluster count =", min(counting$cluster_above_thres))
paste("Mean retained cluster count =", round(mean(counting$cluster_above_thres),1))
paste("Standard deviation of retained cluster count =", round(sd(counting$cluster_above_thres),1))

paste("Maximum reads per well =", max(counting$queries))
paste("Minimum reads per well =", min(counting$queries))
paste("Mean reads per well =", round(mean(counting$queries),1))
paste("Standard deviation of reads per well =", round(sd(counting$queries),1))
paste("Total clustered reads =", sum(counting$queries))
```
...
svg(file="diagrams/well_composition_run2.svg", width=12, height=8)
