...
all.clusters<-ggplot(data = read.stats, aes(x=as.factor(clusters_min_cov), y=cluster_above_thres)) +
  geom_boxplot() +
  labs(y = "Clusters retained (all samples)", x="Minimum cluster coverage") +
...
all.reads<-ggplot(data = read.stats, aes(x=as.factor(clusters_min_cov), y=queries)) +
  geom_boxplot() +
  labs(y = "Per well read depth (all samples)", x="Minimum cluster coverage") +
### make the labels more visible
  theme(axis.text = element_text(size = rel(1.2)),
         axis.title = element_text(size = rel(1.2)))
```
...
neg.clusters<-ggplot(data = subset(read.stats, type=="Negative"), aes(x=as.factor(clusters_min_cov), y=cluster_above_thres)) +
  geom_boxplot() +
  labs(y = "Clusters retained (negatives only)", x="Minimum cluster coverage") +
### make the labels more visible
  theme(axis.text = element_text(size = rel(1.2)),
         axis.title = element_text(size = rel(1.2)))
...
neg.reads<-ggplot(data = subset(read.stats, type=="Negative"), aes(x=as.factor(clusters_min_cov), y=queries)) +
  geom_boxplot() +
  labs(y = "Per well read depth (negatives only)", x="Minimum cluster coverage") +
### make the labels more visible
  theme(axis.text = element_text(size = rel(1.2)),
         axis.title = element_text(size = rel(1.2)))
```
...
g.clusters <- rbind(grob.all.clusters , grob.neg.clusters , size="first")
g.clusters$widths <- unit.pmax(grob.all.clusters$widths, grob.neg.clusters$widths)
grid.newpage()
...
g.reads <- rbind(grob.all.reads , grob.neg.reads , size="first")
g.reads$widths <- unit.pmax(grob.all.reads$widths, grob.neg.reads$widths)
grid.newpage()
